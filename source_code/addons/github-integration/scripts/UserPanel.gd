...
func _connect_signals() -> void:
	...
	RestHandler_.connect("user_repositories_requested", Callable(self, "_on_user_repositories_requested"))
	RestHandler_.connect("user_gists_requested", Callable(self, "_on_user_gists_requested"))
...
func load_icons():
	GistIcon.texture = Github.IconLoaderGithub.load_icon_from_name("gists")
	RepoIcon.texture = Github.IconLoaderGithub.load_icon_from_name("repos")
	ReloadBtn.icon = Github.IconLoaderGithub.load_icon_from_name("reload-gray")
	NewRepo.icon = Github.IconLoaderGithub.load_icon_from_name("repos")
	NewGist.icon = Github.IconLoaderGithub.load_icon_from_name("gists")

# ----- signals 
func _on_request_failed(request_code : int, error_body : Dictionary) -> void:
	match request_code:
		RestHandler_.REQUESTS.USER_REPOSITORIES:
...
func _on_user_repositories_requested(body : Dictionary) -> void:
	...
	if PluginSettings_.owner_affiliations.has("ORGANIZATION_MEMBER"):
...
#	Gists.text = str(UserData.USER.public_gists)
	...
	if not project_repository.is_empty():
		...
		_on_user_repository_requested(await Github.RestHandler_.user_repository_requested)
		
	request_repositories()
	await RestHandler_.user_repositories_requested
	request_gists()
	await RestHandler_.user_gists_requested
...
func request_repositories():
	...
	RestHandler_.request_user_repositories()
...
func request_branches(req : int, rep : Dictionary):
	requesting = req
	request.request("https://api.github.com/repos/"+Github.UserData.USER.login+"/"+rep.name+"/branches",Github.UserData.header,false,HTTPClient.METHOD_GET,"")
