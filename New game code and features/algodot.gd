...
func init():
	...
	get_tree().get_node("/root/Networking").add_child(algod) #makes it a child of the Networking singleton




func test_connections()-> bool: # tests the connections, returns a boolean
	if yield(algod.health(), "completed") == OK:
		return true
	else: return false

func sending_transactions(): #Unorganized
	from_mnemonic = "your twenty five word mnemonic ..."
	from_address = algod.get_address(from_mnemonic)
