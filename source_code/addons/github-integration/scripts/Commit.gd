...
func load_gitignore():
		...
		var ignorefile : FileAccess#= File.new()
		var error #= ignorefile.open(gitignore_filepath+".gitignore",File.WRITE)
...
func request_blobs():
	...
	for file in files:
		...
		if list_file_size[files.find(file)] < 104857600:
			...
			#Github.UserData_.header,false,HTTPClient.METHOD_POST,JSON.stringify(bod))
...
func request_commit_tree():
		...
		#new_repo.request("https://api.github.com/repos/"+repo_selected.owner.login+"/"+repo_selected.name+"/git/trees",Github.UserData_.header,false,HTTPClient.METHOD_POST,JSON.stringify(bod))
		await self.new_tree
		request_new_commit()
...
func request_new_commit():
		...
		#new_repo.request("https://api.github.com/repos/"+repo_selected.owner.login+"/"+repo_selected.name+"/git/commits",Github.UserData_.header,false,HTTPClient.METHOD_POST,JSON.stringify(bod))
...
func request_push_commit():
		...
		#new_repo.request("https://api.github.com/repos/"+repo_selected.owner.login+"/"+repo_selected.name+"/git/refs/heads/"+_branch.get_item_text(branch_idx),Github.UserData_.header,false,HTTPClient.METHOD_POST,JSON.stringify(bod))
		await self.pushed
		...
		if lfs.size() > 0:
				...
				#new_repo.request("https://github.com/"+repo_selected.owner.login+"/"+repo_selected.name+Github.UserData_.gitlfs_request,Github.UserData_.gitlfs_header,false,HTTPClient.METHOD_POST,JSON.stringify(body))
				await self.lfs
		...
		if lfs.size() > 0:
				...
				#new_repo.request("https://github.com/"+repo_selected.owner.login+"/"+repo_selected.name+Github.UserData_.gitlfs_request,Github.UserData_.gitlfs_header,false,HTTPClient.METHOD_PUT,JSON.stringify(body))
				await self.lfs_push
