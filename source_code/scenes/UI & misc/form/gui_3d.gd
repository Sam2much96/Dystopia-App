...
func _unhandled_input(event):
	...
	if event is InputEventMouseButton or InputEventMouseMotion or InputEventScreenDrag or InputEventScreenTouch:#mouse_event:
		is_mouse_event = true
		#break
...
func handle_mouse(event):
	...
	# Detect mouse being held to maintain event while outside of bounds. Avoid orphan clicks
...
func find_mouse(global_position):
	...
	query.collision_mask = node_area.collision_layer
...
func rotate_area_to_billboard():
	...
	if billboard_mode > BaseMaterial3D.BILLBOARD_DISABLED:
		...
		look = node_area.global_position + look
