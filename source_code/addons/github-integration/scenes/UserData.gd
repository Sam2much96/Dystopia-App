...
func _ready():
	var plugin_path = ProjectSettings.globalize_path("user://").replace("app_userdata/"+ProjectSettings.get_setting('application/config/name')+"/",PluginSettings_.directory_name)+"/"
...
func save(user : Dictionary, avatar : PackedByteArray, auth : String, token : String, mail : String) -> void:
	...
	if user!=null:
			...
			print_debug("[GitHub Integration] >> ","saved user datas in user folder")
