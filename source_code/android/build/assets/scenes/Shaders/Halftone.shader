...
void fragment(){
	float fSize = float (size); // float version of 'size0
	vec2 ratio = vec2(1.0, TEXTURE_PIXEL_SIZE.x / TEXTURE_PIXEL_SIZE.y); // make surre the dots are going to be 1:1
	...
	float dots = length(fract(UV * fSize * ratio) - vec2(0.5)) * dot_size; // fracts the UV to make it loop, subtract it by half then turn it into cirlce (using length) and finally multiply witth 2 for smaller circle
	...
	dots += value * value_multiplier ; // add dot with pixels which value will shrink them. Also multoply with value multiplier because sometimes its too bright
	dots = pow(dots, 5.0); // make the dots harder but with anti- aliasing (you can use floor function but the edge will be hard)
	dots = clamp(dots, 0.0, 1.0 ); // clamp the result value to avoid artifacts when blended with other objects
	COLOR.rgb = vec3(dots);
	COLOR.a = texture(TEXTURE,UV).a; //the alpha shouldnt be affected too
}