...
#
	...
	#	# Creates predictable enemy behaviour depending on certain parameters
		state = Simulation.Enemy_.proximity_attack_simulation(hitpoints, raycast, player, player.position, self.position , self, enemy_type, state, enemy_distance_to_player, center)
	#	
	if player == null:
