...
# *************************************************
# Features:
# An Input Manager for Better Touch Controls 
# It expands the Input Event Action with Custom Actions
# It then exposes those actions to the scene tree via a global state
# It can only work when the input managaer is an autoload singleton / of the node calling the custom inputs extends this class
# If it is a child node, it would not work
# On pc it need to emulate touch from mouse settings to work
# *************************************************
# To do:
